...
const DraftPublication = ({ onCreatePublication }) => {
  ...
  useEffect(() => {
    const onClickEv = (e) => {
      if (
        draftModalRef.current?.contains &&
        !draftModalRef.current.contains(e.target)
      ) {
  ...
  return (
    <div ref={draftModalRef} className="inline-block">
      <div
        ...
        } button-wrapper rounded-md border-2 border-primary hover:bg-primary hover:bg-opacity-10`}
      ...
      >
        ...
        <h1 className="text-primary font-semibold select-none hidden md:inline-block">
          Drafts
        </h1>
      </div>
      {showDraftModal && (
        ...
        >
          <div className="bg-white rounded-md p-4 h-60 overflow-y-scroll shadow-2xl">
            <h1 className="text-black font-semibold text-xl">My Draft</h1>
            {currentUserDraft.length !== 0 ? (
              ...
              <div className="flex flex-col justify-center items-center h-full -mt-4  text-primary bg-white">
                <svg
